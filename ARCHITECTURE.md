...
Decided 2026-08-25. `Outcome` held every `Call`, and a `Call` holds the full request and
response JSON. Harmless at four small calls and megabytes once a tool loop runs fifteen
turns, on the one event every consumer must hold. It now carries a `Usage` of counts and
token totals plus a `run_id`; the whole `Call` rides the `wrote` event, where a consumer
reads it and lets it go. Rejected: dropping the bodies until the trace module exists,
which would lose the wire JSON the trace is being built to record.
